...
function getSettings(env, logger) {
  const directory = path.join(env.persistentDataPath, 'app');
  ...
  function load() {
    return new Promise((resolve) => {
      fs.access(fileName, (accessError) => {
        if (accessError) {
          logger.info(`settings file doesn't exist [${accessError}]`);
          resolve();
        } else {
  ...
  function update(setting, value) {
    ...
    fs.writeFile(fileName, JSON.stringify(settings), 'utf8', (err) => {
      if (err) {
        console.error(`unable to write settings file [${err}]`);
      }
    });
    return true;
  }
...
module.exports = (container) => {
  container.service('settings', getSettings, 'env', 'logger');
};